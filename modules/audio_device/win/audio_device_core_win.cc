...
struct AudioDeviceHelper : public DeviceHelper<DEVICE_CLASS> {
 ...
 public:
  ...
  virtual int32_t InitMixer() {
    ...
    if (Transporting() || DeviceHelper<DEVICE_CLASS>::GetDevice() == nullptr) {
      return -1;
    }

    if (DeviceHelper<DEVICE_CLASS>::_usingDeviceIndex) {
      int16_t nDevices = DeviceHelper<DEVICE_CLASS>::Devices();
      if (DeviceHelper<DEVICE_CLASS>::_deviceIndex > (nDevices - 1)) {
    ...
    int32_t ret;
    if (DeviceHelper<DEVICE_CLASS>::_usingDeviceIndex) {
      ret = DeviceHelper<DEVICE_CLASS>::SetDevice(
          DeviceHelper<DEVICE_CLASS>::_deviceIndex);
    } else {
      ret = DeviceHelper<DEVICE_CLASS>::SetDevice(
          DeviceHelper<DEVICE_CLASS>::_deviceType);
    }
    if (ret != 0) {
        ...
        Microsoft::WRL::Make<CCompletionDelegate>();
    {
      ...
      DeviceHelper<DEVICE_CLASS>::GetDevice()->get_Id(
          device_id.ReleaseAndGetAddressOf());
...
struct CaptureDeviceInternal
 ...
 public:
  CaptureDeviceInternal(webrtc::Mutex* critSect, uint32_t* sndCardDelay)
  ...
  DWORD Transport() override {
    ...
    //
    REFERENCE_TIME devPeriod;
    devPeriod = 0;
    REFERENCE_TIME devPeriodMin;
    devPeriodMin = 0;
    ...
    endpointBufferSizeMS = 10.0 * ((double)bufferLength / (double)_blockSize);
      ...
      while (keepRecording) {
        ...
        if (SUCCEEDED(hr)) {
          ...
          uint32_t sndCardRecDelay =
              (uint32_t)(((((UINT64)t1.QuadPart * _perfCounterFactor) -
                           recTime) /
                          10000) +
...
struct RenderDeviceInternal
 ...
 public:
  ...
  RenderDeviceInternal(webrtc::Mutex* critSect) : _critSect(critSect) {
    // list of number of channels to use on recording side
    _channelsPrioList[0] = 2;  // stereo is prio 1
    _channelsPrioList[1] = 1;  // mono is prio 2
  }
...
AudioDeviceGeneric::InitStatus AudioDeviceWindowsCore::Init() {
  webrtc::MutexLock lock(&_internal->_critSect);
